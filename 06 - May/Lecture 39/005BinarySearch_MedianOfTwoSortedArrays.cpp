...
double findMedianSortedArrays(vector<int>& nums1, vector<int>& nums2) {
    ...
    while (s <= e) {
        ...
        if (l1 <= r2 and l2 <= r1) {
            // you've found a valid paritition
            if ((m + n) % 2 == 0) {
            ...
            // your parition is not valid
            if (l1 > r2) {
                // decrease l1 i.e. reduce your search space from [s, e] to [s, x-1]
                e = x - 1;
            } else if (l2 > r1) {
                // increase r1 i.e. increase l1 i.e. reduce the search space from [s, e] to [x+1, e]
                s = x + 1;
            }
        }
    }
