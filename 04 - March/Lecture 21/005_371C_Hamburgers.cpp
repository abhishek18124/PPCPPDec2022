...
bool canPolycarpusCook(ll x) {
    ...
    // to cook 'x' hamburgers, Polycarpus needs x.c_b bread, x.c_s sausage & x.c_c cheese slices
    // in the inventory, Polycarpus already has n_b bread, n_s sausage & n_c cheese slices

    ll e_b = max((ll)0, x*c_b - n_b); // to store extra bread   slices required to cook 'x' hamburgers
    ll e_s = max((ll)0, x*c_s - n_s); // to store extra sausage slices required to cook 'x' hamburgers
    ll e_c = max((ll)0, x*c_c - n_c); // to store extra cheese  slices required to cook 'x' hamburgers
