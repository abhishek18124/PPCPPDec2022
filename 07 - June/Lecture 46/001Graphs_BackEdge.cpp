	...
	bool dfsHelper(T source,
		...
		for (T neighbour : neighbourList) {
			if (visited.find(neighbour) == visited.end()) {
				...
				// check source->neighbour, is it a backedge ?

				// check neighbour------>source, is there a directed path ?

				if (stackState.find(neighbour) != stackState.end()) {
					// neighbour is present on the function call
					// therefore there exists a directed path from
					// neighbour to source hence source to neighbour
					// is a backedge
					return true;
				}
