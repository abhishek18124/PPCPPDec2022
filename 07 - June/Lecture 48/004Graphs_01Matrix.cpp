...
// 2. multi-source BFS - time : O(m.n)

int dx[] = {0, 0, -1, 1};
int dy[] = {1, -1, 0, 0};

vector<vector<int>> updateMatrixEfficient(vector<vector<int>>& mat) {
	...
	while (!q.empty()) {
		...
		for (int k = 0; k < 4; k++) {
			...
			if (ii >= 0 and ii < m and jj >= 0 and jj < n and dist[ii][jj] == INT_MAX) {
				// (ii, jj)th vertex is not yet visited
...
int main() {
	...
	vector<vector<int>> dist = updateMatrixEfficient(mat);
